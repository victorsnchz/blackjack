...
class Rules:
    ...
    def __post_init__(self):
        ...
        if self.number_of_decks < GameConfig.MIN_DECKS:
            raise ValueError('Number of decks must be AT LEAST '
                             f'{GameConfig.MIN_DECKS}.')
        ...
        if type(self.number_of_players) is not int:
            raise TypeError('Number of players must be an integer ',
                            f'not {self.number_of_players}.')
        
        if self.number_of_players < GameConfig.MIN_PLAYERS:
            raise ValueError('Number of players must be AT LEAST', 
                             f'{GameConfig.MIN_PLAYERS}.')